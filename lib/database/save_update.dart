      ...
      try {
        ...
        if (!isChecked && todo.toDate.isNotEmpty) {
          ...
          try {
            ...
            debugPrint('Error canceling notification for ID: $savedId: $e\n$stack');
          }
          ...
          try {
            await NotificationService().showScheduledNotifications(
              ...
              date: selectedToDate!,
              repeatInterval: selectedRepeatList ?? 'No repeat',
            );
          } catch (e, stack) {
            debugPrint('Error scheduling notification for ID: $savedId: $e\n$stack');
            // ignore: use_build_context_synchronously
            ScaffoldMessenger.of(context).showSnackBar(
          ...
          // Cancel notification if task is finished
          try {
            await NotificationService().cancelNotifications(savedId);
          } catch (e, stack) {
            debugPrint('Error canceling notification for ID: $savedId: $e\n$stack');
        ...
        debugPrint('Error saving/updating todo: $e\n$stack');
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
